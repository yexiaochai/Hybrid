//
//  MLHybridWebViewController.swift
//  MedLinker
//
//  Created by 蔡杨
...
class MLHybridWebViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        self.webView = MLHybridWebView(frame: CGRectMake(0, viewInitY, self.view.bounds.size.width, self.view.bounds.height - viewInitY))
        self.webView.autoresizingMask = [ .FlexibleHeight, .FlexibleWidth ]
        self.navigationController?.delegate = self
        //手势监听器
        let edgePan = UIScreenEdgePanGestureRecognizer(target: self, action: #selector(MLHybridWebViewController.edgePanGesture(_:)))
    ...
    override func viewWillAppear(animated: Bool) {
        ...
        //加入删除之前页面的逻辑 最多保留3个 MLHybridWebViewController
    ...
    func deleteViewController() {
        ...
        if let viewControllers = self.navigationController?.viewControllers {
            for vc in viewControllers {
                if vc is MLHybridWebViewController {
...
extension MLHybridWebViewController: UINavigationControllerDelegate {
